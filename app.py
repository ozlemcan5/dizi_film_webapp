...
with app.app_context():
    db.create_all()
    # İlk kullanıcı 'admin' ise veya hiç kullanıcı yoksa ilk kayıt olanı admin yapabiliriz
    # Veya manuel olarak kullanıcı adı 'admin' olanı admin yapalım:
    admin_user = User.query.filter_by(username='admin').first()
    if not admin_user:
        hashed_pw = bcrypt.generate_password_hash('*Ozleemm9870').decode('utf-8')
